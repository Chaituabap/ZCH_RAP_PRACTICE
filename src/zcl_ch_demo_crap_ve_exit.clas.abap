CLASS zcl_ch_demo_crap_ve_exit DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_ch_demo_crap_ve_exit IMPLEMENTATION.
  ...
  METHOD if_sadl_exit_calc_element_read~calculate.
    LOOP AT it_requested_calc_elements INTO DATA(ld_virtual_field).
      LOOP AT ct_calculated_data ASSIGNING FIELD-SYMBOL(<ls_calculated_data>).
        ...
        DATA(ls_original) = CORRESPONDING zch_c_RAPCInvoice( it_original_data[ sy-tabix ] ).
        IF ls_original-Partner = '1000000002'.
          <ld_field> = 999.

        ELSE.
          SELECT FROM zch_dmo_position
            FIELDS COUNT( * )
