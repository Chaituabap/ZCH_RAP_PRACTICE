"! @testing ZCH_UI_SIMPLE_PARTNER_O2
CLASS zcl_ch_demo_unit_service DEFINITION  PUBLIC  FINAL  CREATE PUBLIC
FOR TESTING
DURATION SHORT
RISK LEVEL HARMLESS .
  ...
  PRIVATE SECTION.
    TYPES:BEGIN OF ts_key,
            PartnerNumber TYPE ZCH_C_RAPPartner-PartnerNumber,
          END OF ts_key.
...
CLASS zcl_ch_demo_unit_service IMPLEMENTATION.
  METHOD class_setup.
    DATA: lt_partners TYPE STANDARD TABLE OF zch_dmo_partner WITH EMPTY KEY.

    go_environment = cl_cds_test_environment=>create( i_for_entity = 'ZCH_C_RAPPARTNER'
  ...
  METHOD create_test_client.
    ro_result = cl_web_odata_client_factory=>create_v2_local_proxy( VALUE #( service_id = 'ZCH_UI_SIMPLE_PARTNER_O2' service_version = '0001' ) ).
  ENDMETHOD.
  METHOD get_all_partners.

    DATA: lt_result TYPE TABLE OF ZCH_C_RAPPartner.
  ...
  METHOD update_unison.
    ...
    lo_request->set_business_data( VALUE zch_C_RAPPartner( Street = 'Updated' ) ).
    lo_request->execute(  ).

    SELECT SINGLE FROM zch_dmo_partner
    FIELDS *
    WHERE partner = '3000000003'
    INTO @DATA(ld_partner_found).

    cl_abap_unit_assert=>assert_equals( act = ld_partner_found-street exp = 'Updated' ).
  ENDMETHOD.
  METHOD delete_nevada_inc.
    DATA(lo_cut) = create_test_client(  ).

    DATA(lo_request) = lo_cut->create_resource_for_entity_set( 'Partner' )->navigate_with_key( VALUE ts_key( PartnerNumber = '3000000001' ) )->create_request_for_delete( ).
    lo_request->execute(  ).
