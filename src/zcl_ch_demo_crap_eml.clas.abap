CLASS zcl_ch_demo_crap_eml DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_ch_demo_crap_eml IMPLEMENTATION.
  ...
  ENDMETHOD.
  METHOD delete_data.
    DATA: lt_filter TYPE STANDARD TABLE OF zch_R_RAPCInvoice.

    lt_filter = VALUE #( ( Document = '40000000' ) ).

    MODIFY ENTITIES OF zch_r_RAPCInvoice
    ENTITY Invoice
  ...
  METHOD insert_data.
    ...
    DATA lt_new_invoice TYPE TABLE FOR CREATE ZCH_R_RAPCInvoice.
    DATA lt_new_position TYPE TABLE FOR CREATE ZCH_R_RAPCInvoice\_Position.
  ...
  METHOD read_data.
    ...
    DATA: lt_filter TYPE STANDARD TABLE OF zch_r_RAPCInvoice WITH EMPTY KEY.

    lt_filter = VALUE #( ( Document = '30000000'  )
                         ( Document = '30000005' ) ).


    READ ENTITIES OF ZCH_R_RAPCInvoice
    ENTITY Invoice
